...
pub fn unbounded_channel<T>() -> UnboundedChannel<T> {
    UnboundedChannel { shared: UnboundedShared::with_capacity(0) }
}
...
impl<T> UnboundedChannel<T> {
    ...
    /// is closed, ignoring whether there are any remaining **Sender**(s) or not.
    ///
    /// # Panics
    ///
    /// Panics, if more there exists another pending (already polled at least
    /// once) receive [`Future`] for the same channel and at the time this is
    /// called.
...
mod tests {
    ...
    #[test]
    fn try_recv() {
        let chan = super::unbounded_channel::<i32>();
    ...
    #[test]
    fn try_recv_split() {
        let mut chan = super::unbounded_channel::<i32>();
    ...
    fn multiple_recv() {
        futures_lite::future::block_on(async {
            ...
            core::future::poll_fn(|cx| {
                ...
                // second poll would overwrite first waker
                let _ = recv2.as_mut().poll(cx);
